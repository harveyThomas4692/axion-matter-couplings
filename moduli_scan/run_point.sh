...
#
# Usage: moduli_scan/run_point.sh <psi1> <seed> [n_points] [epochs]
...
CLOUD="$REPO/results/moduli_scan/clouds/psi1=${PSI1}"
RUN="$REPO/results/moduli_scan/runs/psi1=${PSI1}_seed=${SEED}"
mkdir -p "$RUN"
...
"$PY" "$REPO/moduli_scan/stage1_yukawa.py" --psi1="$PSI1" --seed "$SEED" \
      --n-points "$NPTS" --epochs "$EPOCHS" --cloud-dir "$CLOUD" --out-dir "$RUN"

"$PY" "$REPO/moduli_scan/stage2_matter.py" --run-dir "$RUN"

"$PY" "$REPO/moduli_scan/stage3_axion.py" --run-dir "$RUN" --psi1="$PSI1" --seed "$SEED"
